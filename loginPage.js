...
function loginValidate() {
  ...
  if (testEmail && testPassword) {
    let userInfo = storeUserData.find(
      (user) => user.mailId == mailIdLogin.value
    );
    if (userInfo) {
...
function validateForm() {
  const testName = /^([A-Za-z0-9]){4,20}$/gm.test(nameSignUp.value);
  const testNum = /^\d+$/.test(mobileNumSignUp.value);
  const testEmail = /^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g.test(mailIdSignUp.value);
  const testPassword = /^(?=.{8,})(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?!.*\s).*$/gm.test(passwordSignUp.value);
  ...
  if (testName && testNum && testEmail && testPassword) {
    ...
    alert(
      "name should have a length of 3-32 .May contain uppercase,lowercase,numerical values,underscores"
    );
  } else if (!testNum) {
    ...
    alert(
      "At least of 8 Character passwords & a mix of uppercase letters, lowercase letters, numbers, and symbols."
    );
  }
}
